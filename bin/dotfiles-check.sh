...
if [ "${modified_files[@]}:" = ":" ]; then
  echo "All dotfiles managed files committed on $(date)" | tee -a $DOTFILES_LOGFILE
else
  for f in ${modified_files[@]}; do
    echo "Dotfiles managed file $f is modified but not committed on $(date)" | tee -a $DOTFILES_LOGFILE
  done
  exit 1
fi
